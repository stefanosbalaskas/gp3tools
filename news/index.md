...
- Hardened the macOS GitHub Actions lane against reproducible `pak`
  ...
  unknown archive types. Linux and Windows retain the standard
  `setup-r-dependencies` path; macOS now installs the same declared
  dependencies through base R/remotes before running the unchanged R CMD
  check gate.
