...
The adapter passes plot arguments through to the corresponding `eyeprocess` function. A manuscript-ready workflow normally combines one geometry/reassignment figure, assignment stability, and coefficient stability; use the surface only when a two-dimensional perturbation grid was part of the declared analysis plan.
...
For a compact reviewer/manuscript package, pair the Gazepoint adapter settings with: (1) a geometry/reassignment diagnostic, (2) assignment stability, and (3) coefficient stability. Add a robustness surface only for a prespecified joint perturbation grid. The visual layer should always be accompanied by the core perturbation audit, failures, convergence, and model-`N` summaries.
...
For manuscript supplements, reviewer responses, or replication handoff, see the companion **Gazepoint AOI Robustness Reporting Bundle** article, which preserves both adapter settings and the delegated `eyeprocess` core evidence.
