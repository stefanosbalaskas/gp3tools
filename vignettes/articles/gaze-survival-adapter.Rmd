...
```{r survival-plot, eval=requireNamespace("eyeprocess", quietly=TRUE), fig.cap="Kaplan–Meier curves for time to first disclosure fixation in the synthetic Gazepoint adapter example."}
eyeprocess::plot_gaze_survival_curve(result$data, group = "condition")
```

## Complementary survival views

```{r survival-incidence-plot, eval=requireNamespace("eyeprocess", quietly=TRUE), fig.cap="Single-event 1-KM view of the synthetic disclosure-inspection process."}
...
```{r survival-hazard-plot, eval=requireNamespace("eyeprocess", quietly=TRUE), fig.cap="Empirical event/risk increments in the synthetic disclosure-inspection process."}
...
The first plot is simply `1 - KM` for one target event; it is not a competing-risks cumulative-incidence estimator. The second is a descriptive event/risk increment view rather than a smoothed continuous hazard estimate.
...
```{r verification-variant, eval=requireNamespace("eyeprocess", quietly=TRUE)}
