...
safe_value <- function(expr) {
  tryCatch(
    ...
    error = function(e) {
      message('Object not created in this build: ', conditionMessage(e))
      NULL
    }
  )
}
...
safe_plot <- function(expr) {
  ...
  if (inherits(out, 'error')) {
    cat('Plot not rendered in this build: ', conditionMessage(out), '\n')
...
The pupil-status plot shows whether samples are observed or interpolated.
...
The preprocessing plot compares raw, cleaned, interpolated, baseline-corrected,
and smoothed pupil series for one synthetic trial.
...
The AOI transition matrix summarises transitions among AOI states by condition.
...
The legacy transition heatmap provides a compact view of transitions computed
from fixation-level AOI sequences.
...
The GCA plot uses a compact synthetic growth-curve data set to illustrate
...
The model-prediction plot shows fitted trends from a simple model and observed
...
The AOI GAMM plot is included when the optional modelling dependencies and
example fit are available in the website build environment.
...
The cluster-results plot is rendered when a compatible cluster-permutation
result can be computed from the compact synthetic data.
...
The multiverse plot summarises alternative preprocessing decisions when a
compact multiverse object can be created in the website build environment.
